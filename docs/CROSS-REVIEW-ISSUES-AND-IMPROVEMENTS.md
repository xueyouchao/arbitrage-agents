...
1. Candidate-pair and opportunity persistence may use mismatched stable IDs.
2. Public opportunity ID semantics may diverge between domain and API/persistence layers.
3. Freshly fetched orderbooks can appear to be from the future because scanner start time is reused as calculation time.
4. Successful scans record `completedAt` as `startedAt`.
5. Scan artifact persistence is non-atomic and can leave partial committed database state.
...
| 8 | Low | Treat venue HTTP clients as infrastructure adapters, not application services. | GPT-5.5 Architecture | HTTP-specific fetch, retry, parsing, and external payload mapping code lives under `venues/application`; `WorkerScanRunner` imports concrete HTTP clients from that path. Files: `src/contexts/venues/application/http-venue-clients.ts`, `src/contexts/venues/application/static-venue-client.ts`, `src/contexts/venues/domain/venue-market.ts`, `src/contexts/scanner/worker-scan-runner.ts`. | Move concrete HTTP venue clients to `src/contexts/venues/infrastructure/` or similar, while keeping the `VenueClient` port in the domain namespace. | Improves boundary naming, but low urgency unless the venue client layer grows. |

## Recommended Implementation Order

| Step | Recommendation | Why |
|---:|---|---|
| 1 | Fix scan timestamps: calculation `now` and successful `completedAt`. | Small, localized correctness fixes with high observability value. |
| 2 | Make completed scan persistence atomic. | Addresses the strongest cross-cutting data consistency concern and aligns with both bug risk R5 and architecture consensus item 2. |
...
| 5 | Decide public opportunity ID semantics. | Prevents API/domain ID drift before external clients depend on the current shape. |
