 ...
 *
 * Kalshi's `placeOrder` returns its own `OrderResult`
 * ({ orderId, status, filledSize, avgFillPrice }). We map a Kalshi "matched"
...
export class KalshiTradingClientAdapter implements TradingClient {
  ...
  async placeOrder(market: string, side: string, price: number, size: number): Promise<LegFillResult> {
    try {
      ...
      const filled = result.filledSize > 0 || /match|fill/i.test(result.status);
      return {
